...
#define TO_SEARCH_DEVICE_UUID "{00001812-0000-1000-8000-00805F9B34FB}" //we use UUID for an HR BLE device 
...
HANDLE GetBLEHandle(__in GUID AGuid)
{
  ...
  for (DWORD i = 0; SetupDiEnumDeviceInterfaces(hDI, NULL, &BluetoothInterfaceGUID, i, &did); i++)
  {
    ...
    if (!SetupDiGetDeviceInterfaceDetail(hDI, &did, NULL, 0, &size, 0))
    {
      ...
      ZeroMemory(pInterfaceDetailData, sizeof(SP_INTERFACE_DEVICE_DETAIL_DATA));
      ...
      printf("error: %s", GetLastErrorAsString().c_str());
    }
  }
